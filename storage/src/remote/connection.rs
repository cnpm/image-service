...
use super::message::*;
use micro_http::{ScmSocket, SysError};
...
impl Endpoint {
    ...
    pub fn recv_data(&mut self, len: usize) -> Result<(usize, Vec<u8>)> {
        ...
        let (bytes, _) = {
            ...
            unsafe { self.sock.recv_with_fds(&mut iovs, &mut [])? }
        };
        Ok((bytes, rbuf))
    }
...
mod tests {
    ...
    fn send_fd() {
        ...
        let (bytes, buf) = slave.recv_data(5).unwrap();
        #[cfg(target_os = "linux")]
        assert_eq!(bytes, 5);
        ...
        let (bytes, buf, files) = slave.recv_into_buf(0x4).unwrap();
        #[cfg(target_os = "linux")]
        assert_eq!(bytes, 3);
        #[cfg(target_os = "linux")]
        assert!(files.is_none());
